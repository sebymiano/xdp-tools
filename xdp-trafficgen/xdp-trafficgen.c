...
static void *run_traffic(void *arg)
{
	...
	do {
		err = xdp_program__test_run(cfg->prog, &opts);
		if (err) {
			pr_warn("Couldn't run trafficgen program: %s\n", strerror(errno));
			break;
		}
		iterations += opts.repeat;
		/* while testing we send one packet at a time, with 200ms in-between */
		if (cfg->num_pkts)
			usleep(200000);
	} while (!status_exited && (!cfg->num_pkts || cfg->num_pkts > iterations));
...
int do_udp(const void *opt, __unused const char *pin_root_path)
{
	...
	if (err) {
		libxdp_strerror(err, buf, sizeof(buf));
		pr_warn("Couldn't load BPF object: %s\n", buf);
		goto out;
	}

	tcfg.prog = prog;

	runner_threads = calloc(sizeof(pthread_t), cfg->threads);
	if (!runner_threads) {
		pr_warn("Couldn't allocate memory\n");
		goto out_destroy;
	}
	t = calloc(sizeof(struct thread_config), cfg->threads);
	if (!t) {
		pr_warn("Couldn't allocate memory\n");
		goto out_destroy;
	}
	...
	for (i = 0; i < cfg->threads; i++) {
		...
		err = pthread_create(&runner_threads[i], NULL, run_traffic, &t[i]);
		if (err < 0) {
